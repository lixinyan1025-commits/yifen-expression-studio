...
- **今日练习**：默认临场模式，学习结束后才出题。准备模式先出题再学习。阅读完整原始笔记，最多选 3 篇；未选择时优先选取非目录页，再按导入时间选取最多 3 篇。
...
内置题为人工编写的现实场景，按笔记关键词匹配，并明确标为「内置场景题」。AI 题通过所选笔记生成；失败可重试或显式使用内置题，不伪称本地题为 AI 生成。
...
- 上传默认关闭。勾选后仅发送所选最多 3 篇笔记，每篇前 8,000 字符、标题和标签；转写发送本次 WAV；分析发送同一 WAV、题目、原始转写和声学事实。正文展示始终完整。
